...
def encode_wav(wav, sr, rep_format="wav"):
    """
    Encode a waveform tensor as base64-encoded WAV/FLAC/etc, fully in memory.

    wav: torch.Tensor [1, samples]
    sr:  sample rate
    rep_format: e.g. "wav", "flac"
    """
    wav_np = wav.squeeze(0).cpu().numpy()

    with io.BytesIO() as wavio:
        # soundfile decides format by the 'format' arg, not extension
        sf.write(wavio, wav_np, sr, format=rep_format.upper())
        wavio.seek(0)
        audio_bytes = wavio.read()
